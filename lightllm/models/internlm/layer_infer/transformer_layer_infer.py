...
class InternlmTransformerLayerInfer(LlamaTransformerLayerInfer):
    ...
    def _get_qkv(
        ...
        q = torch.addmm(
            ...
            out=self._get_buf((input.view(-1, self.embed_dim_).size(0), layer_weight.q_weight_.size(1)), dlike=input),
        )
        torch.addmm(
    ...
    def _get_o(
        ...
        o_tensor = torch.addmm(
            ...
            out=self._get_buf((input.view(-1, self.tp_o_head_num_ * self.head_dim_).size(0), layer_weight.o_weight_.size(1)), dlike=input),
        )
        return o_tensor
