...
class Internlm2TransformerLayerWeight(LlamaTransformerLayerWeight):
    ...
    def _load_ffn_weights(self, weights):
        ...
        if f"model.layers.{self.layer_num_}.feed_forward.w3.weight" in weights:
            self.up_proj = weights[f"model.layers.{self.layer_num_}.feed_forward.w3.weight"][split_inter_size *
        ...
        if f"model.layers.{self.layer_num_}.feed_forward.w1.weight" in weights:
            self.gate_proj = weights[f"model.layers.{self.layer_num_}.feed_forward.w1.weight"][split_inter_size *
                                                                                             self.tp_rank_: split_inter_size * (self.tp_rank_ + 1), :]
