...
class LlamaTransformerLayerInfer(TransformerLayerInferTpl):
    ...
    def _get_buf(self, shape: Union[torch.Size, Iterable[int]], dlike: torch.Tensor, channel: Literal[None, 'A', 'B'] = None):
        """
        Get a static allocated A/B buffer, if 'channel' is set to None, it will auto switch between A/B buffer by sequence.
    ...
    def _att_norm(
        ...
        return rmsnorm_forward(input, weight=layer_weight.att_norm_weight_, eps=self.eps_,
                               out=self._get_buf(input.shape, dlike=input, channel='A'))    # force set channel A for the first buffer in layer

    def _ffn_norm(
        self, input, infer_state: LlamaInferStateInfo, layer_weight: LlamaTransformerLayerWeight
    ) -> torch.Tensor:
        return rmsnorm_forward(input, weight=layer_weight.ffn_norm_weight_, eps=self.eps_,
                               out=self._get_buf(input.shape, dlike=input))
