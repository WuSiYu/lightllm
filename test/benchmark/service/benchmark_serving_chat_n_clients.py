...
def client_thread(thread_id: int, req_queue: Queue[Request], mode: Literal['known_output_len', 'unknown_output_len'] = 'unknown_output_len'):
    # print(f"client #{thread_id} started\n", end='', flush=True)     # thread safe print
    while True:
        try:
            ...
            break
        asyncio.run(send_request(*req, mode, i))
...
def benchmark(
    ...
    threads = [threading.Thread(target=client_thread, args=(i, _req_queue, mode)) for i in range(n_clients)]
...
def main(args: argparse.Namespace):
    ...
    if not args.use_existing_dump:
        ...
        print(f"\nrunning (mode={mode})...")
        benchmark(input_requests, args.num_clients, mode=mode)
